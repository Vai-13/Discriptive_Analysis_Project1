...
df['Employees'] = df['Employees'].replace(',', '').astype(int)
...
#df['Marketvalue'] = df['Marketvalue'].replace('[\$,]', '', regex=True).astype(float)

# Removing percentage signs and converting to float, then to integer
df['Revchange'] = df['Revchange'].str.replace('%', '').astype(float)
df['Prftchange'] = df['Prftchange'].str.replace('%', '').astype(float)
df.head()
